...
class AnalizadorConocimiento:
    ...
    def analizar_archivo(self, ruta_archivo: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analisis = {
                ...
                "fecha_modificacion": datetime.fromtimestamp(os.path.getmtime(ruta_archivo)).isoformat(),
                "interacciones": 0,
                "patrones_venta": 0,
    ...
    def comparar_archivos(self, analisis_archivos: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if comparacion["puntuacion_completitud"]:
            archivo_mas_completo = max(
                comparacion["puntuacion_completitud"].items(),
                key=lambda x: x[1]
            )[0]
    ...
    def analizar_todos(self) -> Dict[str, Any]:
        ...
        # Generar resumen
        total_interacciones = sum(a.get("interacciones", 0) for a in analisis_archivos if "error" not in a)
        total_patrones = sum(a.get("patrones_venta", 0) for a in analisis_archivos if "error" not in a)
        total_productos = sum(a.get("conocimiento_productos", 0) for a in analisis_archivos if "error" not in a)
    ...
    def generar_reporte_texto(self) -> str:
        ...
        if self.reporte["duplicados"]:
            ...
            for dup in self.reporte["duplicados"]:
                reporte.append(f"  {dup['tipo']}: {', '.join(os.path.basename(a) for a in dup['archivos'])}")
            reporte.append("")
