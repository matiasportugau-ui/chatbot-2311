...
class AuditorProductos:
    ...
    def auditar_sistema_cotizaciones(self) -> Set[str]:
        ...
        try:
            with open("sistema_cotizaciones.py", 'r', encoding='utf-8') as f:
                contenido = f.read()
        ...
        for producto in todos_productos:
            if producto not in productos_base and producto:
                productos_faltantes.append({
                    "producto": producto,
                    "en_matriz_precios": producto in [self.normalizar_producto(p) for p in productos_matriz],
                    "en_conocimiento": producto in [self.normalizar_producto(p) for p in productos_conocimiento],
                    "en_escenarios": producto in [self.normalizar_producto(p) for p in productos_escenarios]
                })
    ...
    def generar_reporte_texto(self) -> str:
        ...
        reporte.append(f"  Matriz de precios: {sorted(self.reporte['productos_en_matriz_precios'])}")
        reporte.append(f"  Archivos de conocimiento: {sorted(self.reporte['productos_en_conocimiento'])}")
    ...
    def guardar_reporte(self, archivo_salida: str = "reporte_auditoria_productos.json"):
        ...
        reporte_serializable = {
            ...
            "productos_en_matriz_precios": sorted(list(self.reporte["productos_en_matriz_precios"])),
