...
class ConsolidadorConocimiento:
    ...
    def encontrar_archivos(self) -> list[str]:
        ...
        patrones = ["*conocimiento*.json", "*base_conocimiento*.json"]
    ...
    def consolidar_patrones(self, patrones: list[dict[str, Any]], archivo_origen: str):
        ...
        for patron in patrones:
            ...
            if patron_id in self.patrones_unicos:
                ...
                existente["frecuencia"] = existente.get("frecuencia", 0) + patron.get(
                    "frecuencia", 0
                )
        ...
        for producto_id, conocimiento in productos.items():
            ...
            if "respuestas_efectivas" in conocimiento:
                if producto_id not in self.productos_consolidados:
                    ...
                    existente["respuestas_efectivas"] = list(
                        respuestas_existente | respuestas_nuevo
                    )
    ...
    def consolidar_todos(self) -> dict[str, Any]:
        ...
        for archivo in archivos:
            ...
            if "metricas_evolucion" in datos:
                fecha_actual = self.conocimiento_consolidado.get("metricas_evolucion", {}).get(
                    "fecha_actualizacion", ""
                )
                fecha_nuevo = datos.get("fecha_exportacion", "")
                if fecha_nuevo > fecha_actual:
                    self.conocimiento_consolidado["metricas_evolucion"] = datos[
                        "metricas_evolucion"
                    ]
    ...
    def validar_integridad(self) -> dict[str, Any]:
        ...
        validacion = {"valido": True, "errores": [], "advertencias": [], "estadisticas": {}}
...
def main():
    ...
    print(
        f"  Archivos consolidados: {len(consolidador.conocimiento_consolidado['archivos_consolidados'])}"
    )
