...
class BaseConocimientoDinamica:
    ...
    def _extraer_palabras_clave(self, texto: str) -> List[str]:
        ...
        # Palabras técnicas
        palabras_tecnicas = [
            "conductividad", "resistencia", "durabilidad", "aislamiento",
            "térmico", "acústico", "fuego", "humedad", "instalación"
        ]
        
        # Palabras de negocio
        palabras_negocio = [
            "precio", "costo", "presupuesto", "oferta", "descuento",
            "entrega", "instalación", "garantía", "servicio"
        ]
        
        # Palabras emocionales
        palabras_emocionales = [
            "urgente", "importante", "necesito", "quiero", "mejor",
            "calidad", "confianza", "seguridad", "garantía"
        ]
    ...
    def _identificar_tipo_consulta(self, mensaje: str) -> str:
        ...
        if any(palabra in mensaje_lower for palabra in ["precio", "costo", "cuanto"]):
            return "precio"
        elif any(palabra in mensaje_lower for palabra in ["especificacion", "caracteristica", "tecnico"]):
            return "tecnico"
    ...
    def _actualizar_patrones_venta_exitosos(self, interaccion: InteraccionCliente, palabras_clave: List[str]):
    ...
    def _actualizar_conocimiento_productos(self, interaccion: InteraccionCliente, palabras_clave: List[str]):
    ...
    def _extraer_objecion(self, mensaje: str) -> Optional[str]:
        """Extrae objeciones del mensaje del cliente"""
        objeciones_comunes = [
            "muy caro", "muy costoso", "no tengo presupuesto",
            "no estoy seguro", "necesito pensarlo", "no es urgente",
            "ya tengo proveedor", "no me convence"
        ]
    ...
    def _extraer_caso_uso(self, interaccion: InteraccionCliente) -> Optional[str]:
    ...
    def _generar_recomendacion_venta(self, interaccion: InteraccionCliente) -> Optional[str]:
    ...
    def _actualizar_metricas_evolucion(self):
        ...
        satisfacciones = [i.satisfaccion_cliente for i in self.interacciones if i.satisfaccion_cliente]
    ...
    def _limpiar_conocimiento_obsoleto(self):
        ...
        ahora = datetime.datetime.now()
        insights_validos = [
            i for i in self.insights_automaticos 
            if (ahora - i['timestamp']).days <= 30
        ]
    ...
    def obtener_respuesta_inteligente(self, mensaje_cliente: str, contexto: Dict[str, Any]) -> str:
        ...
        for interaccion in self.interacciones:
            if (interaccion.resultado == "exitoso" and 
                any(palabra in interaccion.mensaje_cliente.lower() for palabra in palabras_clave)):
    ...
    def _generar_respuesta_basada_en_patron(self, patron: PatronVenta, mensaje: str, contexto: Dict[str, Any]) -> str:
    ...
    def exportar_conocimiento(self, archivo: str):
        ...
        conocimiento_exportar = {
            ...
            "conocimiento_productos": {k: asdict(v) for k, v in self.conocimiento_productos.items()},
    ...
    def _parse_datetime(self, valor: Any) -> datetime.datetime:
        ...
        if isinstance(valor, str):
            ...
            if isinstance(valor, str):
                # Try fromisoformat for full ISO 8601 support including +/- timezone
                if '+' in valor or valor.endswith('Z') or '-' in valor[-6:]:
                    try:
                        from datetime import datetime as dt
                        # Replace Z with +00:00 for compatibility
                        normalized = valor.replace('Z', '+00:00')
