...
try:
    ...
    if os.path.exists(".env.local"):
        ...
        env_cargado = True
        print("[INFO] Variables de entorno cargadas desde .env.local", file=sys.stderr)
    elif os.path.exists(".env"):
        load_dotenv(".env", override=True)
        env_cargado = True
        print("[INFO] Variables de entorno cargadas desde .env", file=sys.stderr)
    else:
        ...
        if result:
            env_cargado = True
            print(
                "[INFO] Variables de entorno cargadas desde archivo .env encontrado",
                file=sys.stderr,
            )
except ImportError:
    print(
        "[WARNING] python-dotenv no instalado. Instala con: pip install python-dotenv",
        file=sys.stderr,
    )
    print("[INFO] Las variables de entorno deben estar configuradas en el sistema", file=sys.stderr)
        ...
        try:
            ...
            if service_account_email and private_key:
                ...
                try:
                    ...
                    print(
                        "[OK] Credenciales de Google Sheets configuradas desde variables de entorno",
                        file=sys.stderr,
                    )
                    return
                except Exception as e:
                    print(
                        f"[WARNING] Error usando credenciales de variables de entorno: {e}",
                        file=sys.stderr,
                    )

            # Intentar leer desde archivo JSON (desarrollo local)
            archivos_credenciales = [
                "credenciales.json",
                ...
                if os.path.exists(archivo):
                    try:
                        self.credenciales = Credentials.from_service_account_file(
                            archivo, scopes=scope
                        )
                        print(
                            f"[OK] Credenciales de Google Sheets cargadas desde {archivo}",
                            file=sys.stderr,
                        )
                        return
                    except Exception as e:
            ...
            print("[WARNING] No se encontraron credenciales de Google Sheets", file=sys.stderr)
            print("[INFO] Usando modo simulado. Para produccion:", file=sys.stderr)
            print(
                "[INFO] 1. Configurar GOOGLE_SERVICE_ACCOUNT_EMAIL y GOOGLE_PRIVATE_KEY en variables de entorno",
                file=sys.stderr,
            )
            print(
                "[INFO] 2. O colocar archivo credenciales.json en la raiz del proyecto",
                file=sys.stderr,
            )
            self.credenciales = None

        except Exception as e:
            print(f"[ERROR] Error configurando credenciales: {e}", file=sys.stderr)
            print("[INFO] Usando modo simulado", file=sys.stderr)
            self.credenciales = None

    def conectar_google_sheets(self):
        """Conecta con Google Sheets"""
        if not self.credenciales:
            print("[WARNING] Credenciales no configuradas, usando modo simulado", file=sys.stderr)
            self.conectado = False
            return False
        ...
        try:
            ...
            self.conectado = True
            print("[OK] Conectado a Google Sheets exitosamente", file=sys.stderr)
            return True

        except Exception as e:
            print(f"[ERROR] Error conectando a Google Sheets: {e}", file=sys.stderr)
            self.conectado = False
            return False
    ...
    def guardar_cotizacion_en_sheets(self, cotizacion_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            codigo_arg = cotizacion_data.get("arg") or self.generar_codigo_arg(
                cotizacion_data.get("telefono", "0000"), cotizacion_data.get("origen", "CH")
            )
            ...
            return {"exito": False, "error": error_msg, "modo": "error"}

    def construir_consulta_cotizacion(
        self, datos_cliente: Dict, datos_especificaciones: Dict
    ) -> str:
