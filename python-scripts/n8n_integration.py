...
try:
    ...
    print(
        json.dumps({"error": f"Error importando módulos del sistema: {str(e)}", "success": False})
    )
    sys.exit(1)
...
def procesar_mensaje(data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        respuesta = ia.procesar_mensaje_usuario(
            mensaje=mensaje, telefono_cliente=telefono or "default", sesion_id=sesion_id
        )
        ...
        return {"error": f"Error procesando mensaje: {str(e)}", "success": False}
...
def crear_cotizacion(data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Error creando cotización: {str(e)}", "success": False}
...
def obtener_insights() -> dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Error obteniendo insights: {str(e)}", "success": False}
...
def analisis_conversiones() -> dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Error en análisis de conversiones: {str(e)}", "success": False}
...
def listar_cotizaciones(data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "data": cotizaciones, "count": len(cotizaciones)}
    except Exception as e:
        return {"error": f"Error listando cotizaciones: {str(e)}", "success": False}
...
def obtener_productos() -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "data": productos, "count": len(productos)}
    except Exception as e:
        return {"error": f"Error obteniendo productos: {str(e)}", "success": False}
...
def main():
    ...
    try:
        ...
        print(json.dumps({"error": f"JSON inválido en input: {str(e)}", "success": False}))
        sys.exit(1)
    except Exception as e:
        print(json.dumps({"error": f"Error general: {str(e)}", "success": False}))
        sys.exit(1)
