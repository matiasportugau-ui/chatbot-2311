...
CAMPOS_OBLIGATORIOS = ["nombre", "apellido", "telefono", "producto", "espesor", "largo", "ancho"]
...
def formatear_mensaje_faltantes(faltantes: list[str]) -> str:
    ...
    if len(faltantes) == 1:
        ...
        if campo == "producto":
            return f"Para poder cotizar necesito que me indiques {descripcion}. ¿Cuál te interesa?"
        elif campo == "espesor":
            return f"Para poder cotizar necesito saber {descripcion}. ¿Qué espesor necesitas?"
        elif campo == "dimensiones":
            return f"Para poder cotizar necesito {descripcion}. ¿Cuáles son las dimensiones?"
        elif campo == "telefono":
            return (
                f"Para poder cotizar necesito {descripcion} para contactarte. ¿Cuál es tu teléfono?"
            )
    ...
    return (
        f"Para poder cotizar necesito los siguientes datos: {lista_texto}. "
        "¿Podrías indicarme esa información?"
    )


def validar_datos_completos(contexto: dict) -> tuple[bool, str | None]:
    """
