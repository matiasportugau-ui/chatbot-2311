...
class SistemaActualizacionAutomatica:
    ...
    def iniciar_sistema_actualizacion(self):
        ...
        # Programar tareas
        schedule.every(self.configuracion["frecuencia_actualizacion_minutos"]).minutes.do(self._actualizar_conocimiento)
        schedule.every(self.configuracion["frecuencia_analisis_horas"]).hours.do(self._analizar_tendencias)
        schedule.every(self.configuracion["frecuencia_limpieza_dias"]).days.do(self._limpiar_datos_obsoletos)
        schedule.every(self.configuracion["frecuencia_exportacion_horas"]).hours.do(self._exportar_conocimiento)
    ...
    def _actualizar_patrones_respuesta(self):
        ...
        respuestas_exitosas = [
            i for i in interacciones_recientes
    ...
    def _generar_nuevos_insights(self) -> int:
        ...
        satisfacciones_recientes = [
            ...
            if (i.satisfaccion_cliente and 
                (datetime.datetime.now() - i.timestamp).days <= 7)
        ]
        ...
        consultas_recientes = [
            i for i in self.base_conocimiento.interacciones
            if (i.tipo_interaccion == "consulta" and 
                (datetime.datetime.now() - i.timestamp).days <= 7)
        ]
    ...
    def _calcular_metricas_actualizadas(self) -> Dict[str, Any]:
        ...
        interacciones_hoy = [
            i for i in self.base_conocimiento.interacciones
            if i.timestamp.date() == ahora.date()
        ]
        
        interacciones_semana = [
            i for i in self.base_conocimiento.interacciones
            if (ahora - i.timestamp).days <= 7
        ]
        
        # Métricas de satisfacción
        satisfacciones = [i.satisfaccion_cliente for i in self.base_conocimiento.interacciones if i.satisfaccion_cliente]
        satisfaccion_promedio = statistics.mean(satisfacciones) if satisfacciones else 0
        
        # Métricas de conversión
        total_cotizaciones = len([i for i in self.base_conocimiento.interacciones if i.tipo_interaccion == "cotizacion"])
        total_ventas = len([i for i in self.base_conocimiento.interacciones if i.tipo_interaccion == "venta"])
    ...
    def _generar_recomendaciones(self) -> List[str]:
        ...
        if metricas["satisfaccion_promedio"] < self.configuracion["umbral_satisfaccion_minimo"]:
            recomendaciones.append("Mejorar calidad de respuestas para aumentar satisfacción del cliente")
        ...
        patrones_exitosos = [p for p in self.base_conocimiento.patrones_venta if p.tasa_exito >= 0.7]
        if patrones_exitosos:
            recomendaciones.append(f"Replicar estrategias de {len(patrones_exitosos)} patrones exitosos identificados")
    ...
    def _analizar_tendencias(self):
        ...
        try:
            ...
            self.logger.info(f"Análisis completado: {len(tendencias)} tendencias, {len(perfiles)} perfiles")
    ...
    def _limpiar_datos_obsoletos(self):
        ...
        try:
            ...
            fecha_limite = datetime.datetime.now() - datetime.timedelta(days=self.configuracion["dias_retencion_datos"])
            
            # Limpiar interacciones antiguas
            interacciones_validas = [
                i for i in self.base_conocimiento.interacciones
                if i.timestamp >= fecha_limite
            ]
            
            interacciones_eliminadas = len(self.base_conocimiento.interacciones) - len(interacciones_validas)
            self.base_conocimiento.interacciones = interacciones_validas
            
            # Limpiar insights antiguos
            insights_validos = [
                i for i in self.base_conocimiento.insights_automaticos
                if datetime.datetime.fromisoformat(i['timestamp']) >= fecha_limite
            ]
            
            insights_eliminados = len(self.base_conocimiento.insights_automaticos) - len(insights_validos)
            self.base_conocimiento.insights_automaticos = insights_validos
            
            self.logger.info(f"Limpieza completada: {interacciones_eliminadas} interacciones, {insights_eliminados} insights eliminados")
    ...
    def _exportar_metricas_sistema(self):
        ...
        with open(self.configuracion["archivos_exportacion"]["metricas_sistema"], 'w', encoding='utf-8') as f:
    ...
    def obtener_estado_sistema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ultima_actualizacion": self.actualizaciones_realizadas[-1].timestamp.isoformat() if self.actualizaciones_realizadas else None,
