...
class AnalizadorEscenarios:
    ...
    def analizar_todos(self) -> Dict[str, Any]:
        ...
        total_escenarios = sum(a.get("numero_escenarios", 0) for a in analisis_archivos if "error" not in a)
        total_mensajes = sum(
            sum(s.get("mensajes", 0) for s in a.get("escenarios", []))
            for a in analisis_archivos if "error" not in a
        )
    ...
    def generar_reporte_texto(self) -> str:
        ...
        for analisis in self.reporte["escenarios"]:
            if "error" in analisis:
                ...
                reporte.append(f"     Productos: {', '.join(analisis.get('productos_mencionados', []))}")
