...
export default function SimulatorPage() {
  ...
  // Normalizar la URL base de la API. Si NEXT_PUBLIC_API_URL o PY_CHAT_SERVICE_URL no están definidos,
  // se usa http://localhost:8000 como predeterminado.
  const API_BASE_URL =
    process.env.NEXT_PUBLIC_API_URL ??
    process.env.PY_CHAT_SERVICE_URL ??
    "http://localhost:8000";

  // Eliminar cualquier barra final de API_BASE_URL y concatenar una sola vez el prefijo /api/chat.
  function buildChatEndpoint(): string {
    return `${API_BASE_URL.replace(/\/+$/, "")}/api/chat`;
  }
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL.replace(/\/+$/, "")}/conversations`);
      if (response.ok) {
  ...
  const sendMessage = async () => {
    ...
    const loadConversation = async (sessionId: string) => {
      try {
        const response = await fetch(`${API_BASE_URL.replace(/\/+$/, "")}/conversations/${sessionId}`);
        if (response.ok) {
          const data = await response.json();
