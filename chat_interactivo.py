...
try:
    ...
    from language_processor import Intent as LPIntent
...
class AgenteInteractivo:
    ...
    def __init__(self):
        ...
        if LANGUAGE_PROCESSOR_AVAILABLE:
            try:
                ...
                self.use_language_processor = True
            except Exception as e:
                print(f"Warning: Could not initialize language processor: {e}")
                self.language_processor = None
                self.use_language_processor = False
        else:
            self.language_processor = None
            self.use_language_processor = False
        ...
        try:
            ...
            print(f"⚠️  No se pudo inicializar Google Sheets: {e}", file=sys.stderr)
    ...
    def cargar_configuracion(self):
        ...
        # Redirect system messages to stderr to avoid interfering with input()
        print("[AGENTE] Agente de Cotizaciones BMC Uruguay iniciado", file=sys.stderr)
        print("[SISTEMA] Sistema cargado y listo para atenderte", file=sys.stderr)

    def procesar_mensaje(self, mensaje: str):
        """Procesa el mensaje del usuario y responde"""

        # Use centralized language processor if available
        if self.use_language_processor and self.language_processor:
            try:
                processed = self.language_processor.process_message(mensaje)
                intent = processed.intent

                # Route based on detected intent
                if intent == LPIntent.SALUDO:
                    return self.saludar()
                elif intent == LPIntent.COTIZACION:
                    return self.iniciar_cotizacion()
                elif intent == LPIntent.PRODUCTO or intent == LPIntent.INFORMACION:
                    return self.responder_consulta_producto(mensaje.lower())
                elif intent == LPIntent.DESPEDIDA:
                ...
                return self._procesar_mensaje_legacy(mensaje)
        else:
            # Use legacy pattern matching
            return self._procesar_mensaje_legacy(mensaje)

    def _procesar_mensaje_legacy(self, mensaje: str):
        """Legacy message processing using pattern matching"""
        ...
        # Detectar intención de cotizar
            ...
            for palabra in ["cotizar", "precio", "costo", "cuanto", "cotizacion"]
        ):
            return self.iniciar_cotizacion()
        ...
        # Detectar consulta sobre productos
            ...
            palabra in mensaje_lower
            for palabra in ["isodec", "poliestireno", "lana", "producto", "productos"]
        ):
        ...
        # Detectar despedida
            ...
            palabra in mensaje_lower
            for palabra in ["gracias", "chau", "adios", "bye", "hasta luego"]
        ):
            return self.despedir()
    ...
    def procesar_nombre_apellido(self, mensaje):
        ...
        if len(partes) >= 2:
            ...
            self.paso_actual = 2
            return (
                f"¡Hola {self.datos_cliente['nombre']} {self.datos_cliente['apellido']}! 👋\n\n"
                "**PASO 2 - CONTACTO**\n"
    ...
    def procesar_producto(self, mensaje):
        ...
        if "isodec" in mensaje_lower:
            ...
            return (
                "❌ Por favor, selecciona uno de los productos: Isodec, Poliestireno o Lana de Roca"
            )

        self.datos_especificaciones["producto"] = producto
        self.paso_actual = 5
        return (
            f"✅ Producto seleccionado: {producto.upper()}\n\n"
            "**PASO 5 - DIMENSIONES**\n"
        ...
        if len(numeros) >= 2:
            try:
                ...
                if largo > 0 and ancho > 0:
                    ...
                    self.paso_actual = 6
                    return (
                        f"✅ Dimensiones registradas: {largo}m x {ancho}m\n\n"
                        "**PASO 6 - ESPESOR**\n"
                ...
                return (
                    "❌ Por favor, ingresa las dimensiones en formato: largo x ancho (ej: 10 x 5)"
                )
        else:
    ...
    def procesar_espesor(self, mensaje):
        ...
        if espesor and espesor[0] in ["50", "75", "100", "125", "150"]:
            ...
            self.paso_actual = 7
            return (
                f"✅ Espesor registrado: {self.datos_especificaciones['espesor']}\n\n"
                "**PASO 7 - COLOR**\n"
                "¿Qué color prefieres?\n"
                "• Blanco\n"
                "• Gris\n"
    ...
    def finalizar_cotizacion(self):
        ...
        try:
            ...
            contexto_validacion = construir_contexto_validacion(
                self.datos_cliente, self.datos_especificaciones
            )
    ...
    def despedir(self):
        ...
        return (
            "👋 ¡Gracias por contactar BMC Uruguay!\n\n"
...
def main():
    ...
    print("=" * 70)
    sys.stdout.flush()  # Ensure header is printed before initialization
    ...
    while True:
        try:
            ...
            sys.stdout.flush()  # Flush before input
            mensaje = input("\n👤 Tú: ").strip()
