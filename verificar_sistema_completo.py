...
def check_api_server() -> dict[str, Any]:
    """Check if API server is running by hitting health endpoint"""
    try:
        import requests # Import requests locally to handle its optional nature
        response = requests.get("http://localhost:8000/health", timeout=2)
        if response.status_code == 200:
            return {"running": True, "status": "healthy", "details": response.json()}
        else:
            return {"running": True, "status": "error", "code": response.status_code}
    except ImportError:
        return {"running": False, "status": "not_available"}
    except Exception:
...
def print_results():
    ...
    elif mongodb_status["docker_available"]:
        print("  - MongoDB: ⚠️  Docker disponible pero MongoDB no está corriendo")
    else:
        print("  - MongoDB: ⏭️  No disponible (opcional)")
    print()
