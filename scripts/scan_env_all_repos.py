...
Usage:
    GH_OWNER=your_username GH_SCAN_TOKEN=your_token python scripts/scan_env_all_repos.py

Environment Variables:
    GH_OWNER: GitHub username or organization name
    GH_SCAN_TOKEN: GitHub token with read-only access to repos
    REPO_FILTER: (optional) Filter repos by name substring
    BRANCH_LIMIT: (optional) Max branches per repo (0 = unlimited)
    BRANCH_ALLOW_REGEX: (optional) Regex to filter branches
...
def list_repos(owner: str, token: str, repo_filter: Optional[str]) -> List[dict]:
    ...
    page = 1
    while True:
        # For large orgs, you may need: f"{API}/orgs/{owner}/repos"
...
def extract_keys_from_text(txt: str) -> List[str]:
    """Extract environment variable keys from text (never values)."""
    keys = set()
...
def scan_repo(
    ...
    """Scan a repository for .env files across all branches."""
...
def build_unified_example(inventory: List[Dict]) -> tuple:
    """Build a unified .env.example from all discovered keys."""
    all_keys = set()
