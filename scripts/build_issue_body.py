#!/usr/bin/env python3
"""Builds daily issue body from outputs.

Security: does not read secrets; only consumes generated artifacts.
"""
...
def main() -> None:
    ...
    if opp_json.exists():
        ...
        for i, o in enumerate(opp["top"][:10], 1):
            lines.append(
                f"{i}. **{o['producto']}** — score {o['score']} — riesgo {o['riesgo']} — bundle: {o['bundle']}"
            )
    else:
    ...
    lines.append("- Ajustar `{BRANCH_ALLOW_REGEX}` y/o `{BRANCH_LIMIT}` para performance.")
