...
class EnvironmentWizard:
    ...
    def prompt_required(self):
        ...
        for var, config in required.items():
            ...
            if current and not current.startswith("your-") and current:
                ...
                response = input("  Keep current value? (y/n, default: y): ").strip().lower()
                if response != "n":
                    continue
                ...
                if value:
                    if "format" in config and config["format"] == "sk-...":
                        if not value.startswith("sk-") or len(value) < 20:
                            print(
                                "  ⚠️  Invalid format. Should start with 'sk-' and be at least 20 characters"
                            )
                            continue
    ...
    def prompt_optional(self):
        ...
        for var, config in optional.items():
            ...
            if "generate" in config and config.get("generate") and not current:
                ...
                response = input("  Use generated value? (y/n, default: y): ").strip().lower()
                if response != "n":
            ...
            if not current or current.startswith("your-"):
                ...
                response = input("  Keep current value? (y/n, default: y): ").strip().lower()
                if response == "n":
                    value = input("  Enter new value (or press Enter to skip): ").strip()
                    if value:
                        self.env_vars[var] = value
    ...
    def run(self):
        ...
        if self.validate_setup():
            ...
            print("  2. Run: python unified_launcher.py --setup-only")
            print("  3. Start the system: python unified_launcher.py\n")
            return True
        else:
