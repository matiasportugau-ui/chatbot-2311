...
class UnifiedModelIntegrator:
    ...
    def _load_unified_config(self):
        """Load unified_config.json if available"""
        try:
            config_path = "unified_config.json"
            if os.path.exists(config_path):
                with open(config_path, "r") as f:
                    unified = json.load(f)
                    # Example: Use unified config to override defaults if needed
                    logger.info(f"✅ Loaded unified configuration from {config_path}")
                    # Logic to apply specific overrides can be added here
        except Exception as e:
            logger.warning(f"Could not load unified_config.json: {e}")
    ...
    def _load_from_env(self, default_configs: Dict):
        ...
        if GROK_AVAILABLE:
            ...
            if grok_key:
                models = os.getenv(
                    "XAI_MODEL", os.getenv("GROK_MODELS", "grok-beta,grok-2-1212,grok-4-latest")
                ).split(",")
                for model in models:
                    model = model.strip()
                ...
                if gemini_configs:
                    ...
                    if GEMINI_AVAILABLE and not GEMINI_USE_OLD_SDK:
                        ...
                        if project_id and api_key:
                            ...
                            structured_logger.info(
                                f"✅ Gemini configured (Vertex AI REST API - project: {project_id})"
                            )
                        elif project_id:
                            ...
                            structured_logger.info(
                                f"✅ Gemini client initialized (Vertex AI SDK - project: {project_id})"
                            )
                        else:
                            self.clients[ModelProvider.GEMINI.value] = genai.Client(api_key=api_key)
                            structured_logger.info(
                                "✅ Gemini client initialized (Google AI Studio)"
                            )
                        ...
                        structured_logger.info(
                            "✅ Gemini client initialized (old SDK - google-generativeai)"
                        )
            except Exception as e:
    ...
    def _select_best_model(self, task_type: str = "general") -> Optional[str]:
        ...
        """
        enabled_models = {
            model_id: config for model_id, config in self.models.items() if config.enabled
        }
        ...
        for model_id, config in enabled_models.items():
            ...
            if self.strategy == "cost":
                ...
                cost_score = 1.0 / (
                    1.0 + config.cost_per_1k_tokens_input + config.cost_per_1k_tokens_output
                )
                ...
                score = cost_score * 0.3 + config.speed_rating * 0.3 + config.quality_rating * 0.4
    ...
    def generate(
        ...
        **kwargs,
        ...
        if request_tracker:
            ...
            set_request_context(request_metadata.request_id, request_metadata.client_request_id)
        ...
        if not model_id or model_id not in self.models:
            ...
            if request_metadata:
                request_tracker.update_request(
                    request_metadata.request_id, status="failed", error=error_msg
                )
        ...
        try:
            ...
            if provider == "openai":
                response = self._generate_openai(
                    ...
                    max_tok,
                    client_request_id=(
                        request_metadata.client_request_id if request_metadata else None
                    ),
            ...
            cost = (tokens_input / 1000) * config.cost_per_1k_tokens_input + (
                tokens_output / 1000
            ) * config.cost_per_1k_tokens_output
            ...
            if UTILS_AVAILABLE and response_headers:
                ...
                if rate_limit_monitor:
                    rate_limit_monitor.update_from_headers(
                        response_headers, provider, config.organization_id
                    )
            ...
            if request_metadata and request_tracker:
                request_tracker.update_request(
                    request_metadata.request_id, status="completed", response_time=response_time
                )
    ...
    def save_usage_stats(self):
        ...
        stats_file = "model_usage_stats.json"
        try:
            data = {model_id: asdict(stats) for model_id, stats in self.usage_stats.items()}
    ...
    def get_usage_summary(self) -> Dict[str, Any]:
        ...
        total_tokens = sum(
            stats.tokens_input + stats.tokens_output for stats in self.usage_stats.values()
        )
        ...
        return {
            ...
            "by_model": {model_id: asdict(stats) for model_id, stats in self.usage_stats.items()},
        }
