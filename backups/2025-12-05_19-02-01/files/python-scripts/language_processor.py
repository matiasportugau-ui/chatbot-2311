...
class EntityExtractor:
    ...
    def extract(
        self, text: str, intent: Intent, language: Language = Language.SPANISH
    ) -> dict[str, Any]:
        """Extract entities from text"""
        entities = {
            "products": [],
            ...
            for keyword in keywords:
                if keyword in text_lower:
                    entities["products"].append(
                        {"id": product_id, "name": keyword, "confidence": 0.9}
                    )
                    break
        ...
        for pattern in self.dimension_patterns:
            ...
            if match:
                if len(match.groups()) == 2:
                    ...
                    entities["dimensions"] = {"area_m2": float(match.group(1)), "confidence": 0.8}
                break

        # Extract thickness
        thickness_match = self.thickness_pattern.search(text)
        if thickness_match:
            entities["thickness"] = {"value": thickness_match.group(1) + "mm", "confidence": 0.9}
        ...
        for color_id, keywords in self.colors.items():
            ...
            for keyword in keywords:
                if keyword in text_lower:
                    entities["color"] = {"value": color_id, "confidence": 0.8}
                    color_found = True
                    break
        # Extract phone
        phone_match = self.phone_pattern.search(text)
        if phone_match:
            entities["phone"] = {"value": phone_match.group(0).replace(" ", ""), "confidence": 0.9}

        # Extract services
        if "flete" in text_lower or "transporte" in text_lower or "delivery" in text_lower:
            entities["services"]["flete"] = True
        if (
            "instalacion" in text_lower
            or "instalación" in text_lower
            or "instalacao" in text_lower
            or "instalação" in text_lower
            or "instalar" in text_lower
...
class CacheManager:
    ...
    def get(self, text: str) -> ProcessedMessage | None:
        """Get cached result"""
    ...
    def set(self, text: str, result: ProcessedMessage):
        ...
        self.cache[key] = {"result": result, "timestamp": datetime.datetime.now()}
...
class ContextManager:
    ...
    def update_context(self, session_id: str, result: ProcessedMessage):
        ...
        self.contexts[session_id] = {"data": context, "last_update": datetime.datetime.now()}
...
class LanguageProcessor:
    ...
    def process_message(self, message: str, session_id: str | None = None) -> ProcessedMessage:
        ...
        )  # Use original for entity extraction
