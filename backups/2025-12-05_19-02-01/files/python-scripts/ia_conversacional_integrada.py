...
class IAConversacionalIntegrada:
    ...
    def _manejar_cotizacion(
        ...
        if contexto.estado_cotizacion == "inicial":
            ...
            if (
                contexto.datos_producto.get("producto")
                and contexto.datos_producto.get("largo")
                and contexto.datos_producto.get("ancho")
            ):
                # Crear cotización
    ...
    def _registrar_interaccion(
        self, mensaje_cliente: str, respuesta: RespuestaIA, contexto: ContextoConversacion
    ):
