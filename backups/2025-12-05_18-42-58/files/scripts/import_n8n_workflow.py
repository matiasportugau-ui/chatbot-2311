...
class N8NWorkflowImporter:
    ...
    def __init__(self, n8n_url: str = "http://localhost:5678", api_key: str | None = None):
    ...
    def list_workflows(self) -> list[dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.n8n_url}/api/v1/workflows", headers=self.get_headers(), timeout=10
            )
    ...
    def import_workflow(self, workflow_file: Path, activate: bool = True) -> dict[str, Any] | None:
    ...
    def import_all_workflows(self, activate: bool = True) -> dict[str, Any]:
        ...
        if not self.workflows_dir.exists():
            ...
            return {"success": False, "imported": [], "errors": self.errors}
    ...
    def save_report(self, result: dict[str, Any], output_file: Path | None = None) -> Path:
...
def main():
    ...
    parser.add_argument("--api-key", help="n8n API key (optional, for authentication)")
    parser.add_argument("--workflow", type=Path, help="Import specific workflow file")
    parser.add_argument(
        "--no-activate", action="store_true", help="Don't activate workflows after import"
    )
    ...
    if args.workflow:
        result = {
            "success": importer.import_workflow(args.workflow, activate=not args.no_activate)
            is not None,
